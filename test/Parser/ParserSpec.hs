  ...
  describe "parseCmdPhrase" $ do
    ...
    context "with verb-target commands" $ do
      ...
      it "handles complex noun phrases in target" $ do
        parseCmdPhrase "look ancient stone altar"
          `shouldBe` Right (UnaryCmdExpression "look" (NounClause "ancient stone altar"))
        parseCmdPhrase "get mysterious glowing runes"
          `shouldBe` Right (UnaryCmdExpression "get" (NounClause "mysterious glowing runes"))

    context "with preposition commands" $ do
      it "parses simple preposition commands" $ do
        parseCmdPhrase "look at bag"
          `shouldBe` Right (SplitCmdExpression "look" (PrepClause "at") (NounClause "bag"))
        parseCmdPhrase "look in box"
          `shouldBe` Right (SplitCmdExpression "look" (PrepClause "in") (NounClause "box"))

      it "handles equivalent prepositions" $ do
        parseCmdPhrase "look inside box"
          `shouldBe` Right (SplitCmdExpression "look" (PrepClause "in") (NounClause "box"))
        parseCmdPhrase "look into box"
          `shouldBe` Right (SplitCmdExpression "look" (PrepClause "in") (NounClause "box"))

      it "handles complex phrases with prepositions" $ do
        parseCmdPhrase "look at ancient stone altar"
          `shouldBe` Right (SplitCmdExpression "look" (PrepClause "at") (NounClause "ancient stone altar"))
        parseCmdPhrase "look under weathered marble statue"
          `shouldBe` Right (SplitCmdExpression "look" (PrepClause "under") (NounClause "weathered marble statue"))

    context "with complex object commands" $ do
      it "parses basic object placement" $ do
        parseCmdPhrase "put bauble in bag"
          `shouldBe` Right (ComplexCmdExpression "put" (NounClause "bauble") (PrepClause "in") (NounClause "bag"))
        parseCmdPhrase "move key to box"
          `shouldBe` Right (ComplexCmdExpression "move" (NounClause "key") (PrepClause "to") (NounClause "box"))

      it "handles articles in both object and target" $ do
        parseCmdPhrase "put the bauble in the bag"
          `shouldBe` Right (ComplexCmdExpression "put" (NounClause "bauble") (PrepClause "in") (NounClause "bag"))
        parseCmdPhrase "move the orb onto the pedestal"
          `shouldBe` Right (ComplexCmdExpression "move" (NounClause "orb") (PrepClause "on") (NounClause "pedestal"))

      it "handles complex phrases in both object and target" $ do
        parseCmdPhrase "put golden orb on marble pedestal"
          `shouldBe` Right (ComplexCmdExpression "put" (NounClause "golden orb") (PrepClause "on") (NounClause "marble pedestal"))
        parseCmdPhrase "place ancient brass key in rusty lock"
          `shouldBe` Right (ComplexCmdExpression "place" (NounClause "ancient brass key") (PrepClause "in") (NounClause "rusty lock"))
