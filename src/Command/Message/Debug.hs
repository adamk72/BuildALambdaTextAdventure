...
formatLocation loc =
  "\n  Location: "
    <> formatEntityBase (locationBase loc)
    <> "\n    Destinations: "
    <> T.intercalate ", " (map unEntityId $ destinations loc)

formatActor :: Entity 'ActorT -> Text
formatActor actor =
  "\n  Actor: "
    <> formatEntityBase (actorBase actor)
    <> "\n    Location: "
    <> unEntityId (actorLocationId actor)

formatItem :: Entity 'ItemT -> Text
formatItem item =
  "\n  Item: "
    <> formatEntityBase (itemBase item)
    <> "\n    Container: "
    <> unEntityId (itemLocationId item)
