...
module Stories.Trial(gameIntro, allVerbs, allNouns, allPrepositions, allTokens, startScene, startInventory, startFlags, defaultScene, allScenes) where

import           Data.Map              (Map, fromList)

import           NarrativeGraph        (ConditionalAction (ConditionalAction, condition, conditionalDescription, stateChanges),
                                        ConditionalDescription (ConditionalDescription),
                                        Flags (..),
                                        Interaction (Interaction, conditionalActions, sentences),
                                        Inventory (..),
                                        NarrativeCondition (CAnd, CNot, CTrue, FlagSet, InInventory),
                                        Scene (..), SceneKey,
                                        StateChange (AddToInventory, RemoveFlag, SceneChange, SetFlag))
